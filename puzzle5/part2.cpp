...
int main(int argc, char** argv)
{
    ...
    for (int n = 0; n < numCoords; ++n) {
        ...
        } else if (std::abs(startCoord[n].first - endCoord[n].first)
                == std::abs(startCoord[n].second - endCoord[n].second)) {
            auto xmove = endCoord[n].first - startCoord[n].first;
            auto ymove = endCoord[n].second - startCoord[n].second;
            if (xmove > 0 && ymove > 0)
                for (int i = 0; i <= xmove; ++i)
                    board[startCoord[n].second + i][startCoord[n].first + i]++;
            if (xmove > 0 && ymove < 0)
                for (int i = 0; i <= xmove; ++i)
                    board[startCoord[n].second - i][startCoord[n].first + i]++;
            if (xmove < 0 && ymove > 0)
